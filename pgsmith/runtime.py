"""PostgreSQL runtime orchestration."""
...
from pgsmith import random_utils
from pgsmith.exceptions import DutBroken, DutFailure
from pgsmith.logger import CerrLogger, ImpedanceFeedback, Logger, QueryDumper
from pgsmith.relmodel import Scope
...
def build_runtime(config: RunConfig) -> RuntimeContext:
    ...
    print(f"pysqlsmith {VERSION} [postgres] (seed={seed_val})", file=sys.stderr)

    from .schema import SchemaPostgres

    # Schema loading is the expensive one-time setup step. After this, generation is in-memory.
    schema = SchemaPostgres(config, exclude_catalog=config.exclude_catalog)
    schema.summary()
