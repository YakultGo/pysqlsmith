"""PostgreSQL statement-level grammar productions."""

from __future__ import annotations
from typing import Optional, List

from pgsmith.prod import Prod
from pgsmith.relmodel import (
    SQLType, Column, Relation, NamedRelation, AliasedRelation, Table, Scope,
)
from pgsmith.expr import ValueExpr, BoolExpr, ColumnReference, WindowFunction, ConstExpr, ForeignKeySubselect
from pgsmith.random_utils import d6, d9, d12, d20, d42, d100, random_pick
...
def statement_factory(s: Scope, select_only: bool = False) -> Prod:
    try:
        ...
        if d42() == 1 and not select_only and getattr(schema, "version_num", 0) >= 150000:
            return MergeStmt(None, s)
        if d42() == 1 and not select_only:
            return UpsertStmt(None, s) if d6() == 1 else InsertStmt(None, s)
        if d42() == 1 and not select_only:
            return DeleteReturning(None, s) if d6() == 1 else DeleteStmt(None, s)
        if d42() == 1 and not select_only:
            return UpdateReturning(None, s) if d6() == 1 else UpdateStmt(None, s)
        if d6() > 2:
